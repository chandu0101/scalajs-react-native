...
import chandu0101.scalajs.rn.ReactNativeComponentB.{fcEither, fcUnit}
import japgolly.scalajs.react._

import scala.scalajs.js
import scala.scalajs.js.{Any => JAny, Dynamic, ThisFunction, UndefOr, undefined,Array => JArray}
...
object ReactNativeComponentB {
  ...
  implicit def defaultProps[P,S,B,N <: TopNode](c: ReactNativeComponentB[P,S,B,N]) = c.propsRequired
  ...
  // ===================================================================================================================
  final class PSBN[P, S, B] private[ReactNativeComponentB](name: String, initF: P => S, backF: BackendScope[P, S] => B, rendF: ComponentScopeU[P, S, B] => ReactElement) {
    def domType[N <: TopNode]: ReactNativeComponentB[P, S, B, N] =
      new ReactNativeComponentB(name, initF, backF, rendF, emptyLifeCycle, Vector.empty)
  }
  // ===================================================================================================================
  private[rn] case class LifeCycle[P,S,B,N <: TopNode](
  ...
  private[rn] def emptyLifeCycle[P,S,B,N <: TopNode] =
    LifeCycle[P,S,B,N](undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined)
}

import ReactNativeComponentB.LifeCycle
final class ReactNativeComponentB[P,S,B,N <: TopNode](val name: String,
                                                initF   : P => S,
  ...
  @inline private def copy(name    : String                                   = name    ,
                           ...
                           jsMixins: Vector[JAny]                             = jsMixins): ReactNativeComponentB[P, S, B, N] =
    new ReactNativeComponentB(name, initF, backF, rendF, lc, jsMixins)

  @inline private implicit def lcmod(a: LifeCycle[P, S, B, N]): ReactNativeComponentB[P, S, B, N] =
    copy(lc = a)

  def configureSpec(modify: ReactComponentSpec[P, S, B, N] => Unit): ReactNativeComponentB[P, S, B, N] =
    lc.copy(configureSpec = modify)

  def configure(fs: (ReactNativeComponentB[P, S, B, N] => ReactNativeComponentB[P, S, B, N])*): ReactNativeComponentB[P, S, B, N] =
    fs.foldLeft(this)((a,f) => f(a))

  def getDefaultProps(p: => P): ReactNativeComponentB[P, S, B, N] =
    lc.copy(getDefaultProps = () => p)

  def componentWillMount(f: ComponentScopeU[P, S, B] => Unit): ReactNativeComponentB[P, S, B, N] =
    lc.copy(componentWillMount = fcUnit(lc.componentWillMount, f))

  def componentDidMount(f: ComponentScopeM[P, S, B, N] => Unit): ReactNativeComponentB[P, S, B, N] =
    lc.copy(componentDidMount = fcUnit(lc.componentDidMount, f))

  def componentWillUnmount(f: ComponentScopeM[P, S, B, N] => Unit): ReactNativeComponentB[P, S, B, N] =
    lc.copy(componentWillUnmount = fcUnit(lc.componentWillUnmount, f))

  def componentWillUpdate(f: (ComponentScopeWU[P, S, B, N], P, S) => Unit): ReactNativeComponentB[P, S, B, N] =
    lc.copy(componentWillUpdate = fcUnit(lc.componentWillUpdate, f))

  def componentDidUpdate(f: (ComponentScopeM[P, S, B, N], P, S) => Unit): ReactNativeComponentB[P, S, B, N] =
    lc.copy(componentDidUpdate = fcUnit(lc.componentDidUpdate, f))

  def componentWillReceiveProps(f: (ComponentScopeM[P, S, B, N], P) => Unit): ReactNativeComponentB[P, S, B, N] =
    lc.copy(componentWillReceiveProps = fcUnit(lc.componentWillReceiveProps, f))

  def shouldComponentUpdate(f: (ComponentScopeM[P, S, B, N], P, S) => Boolean): ReactNativeComponentB[P, S, B, N] =
   ...
   */
  def mixinJS(mixins: JAny*): ReactNativeComponentB[P, S, B, N] =
    copy(jsMixins = jsMixins ++ mixins)

  /**
   * Modify the render function.
   */
  def reRender(f: (ComponentScopeU[P, S, B] => ReactElement) => ComponentScopeU[P, S, B] => ReactElement): ReactNativeComponentB[P, S, B, N] =
    copy(rendF = f(rendF))
  ...
  final class Builder[C] private[ReactNativeComponentB](cc: ReactComponentCU[P,S,B,N] => C) {
